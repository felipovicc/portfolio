...
const Skils = () => {
    ...
    const getSkillIcon = (type) => {
        const iconStyle = 'dark:text-green-300 w-6 h-6 flex-shrink-0 mr-4'
        const iconSize = 32
        switch (type) {
            case 'development': return <HiCode size={iconSize} className={iconStyle} />
            case 'design': return <MdDesignServices size={iconSize} className={iconStyle} />
            default: return <HiBadgeCheck size={iconSize} className={iconStyle} />
        }
    }
